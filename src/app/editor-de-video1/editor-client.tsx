...
import { getAllQuotes } from "@/lib/actions"; // Importando a Server Action!
...
export default function EditorClient() {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
        ...
        let text = "A inspiração está a caminho..."; // Texto padrão
        if (quoteParam) {
            ...
            try {
                ...
                console.error("Falha ao buscar frases do servidor:", error);
                // O texto padrão será usado como fallback
            }
        }
  ...
  return (
    <div className="flex flex-col w-full bg-background font-body text-foreground h-full">
      {isDesktop ? (
        ...
        <main className="flex-1 w-full h-full overflow-auto flex items-center justify-center p-4">
          <PreviewCanva {...previewProps} />
        </main>
      )}
      {!isDesktop && <MobileToolbar {...commonProps} />}
    </div>
  );
}
